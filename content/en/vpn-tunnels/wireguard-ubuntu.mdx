...
### Dynamic IP Issues
Most UK ISPs provide dynamic IPs. Solutions:
- Use Dynamic DNS (DuckDNS, No-IP)
- Check IP regularly: `curl ifconfig.me`

### CGNAT Problems (Three, some Virgin areas)
If you're behind CGNAT:
- Contact ISP for static IP (£5-10/month)
- Use reverse tunnel (Cloudflare Tunnel)
- Consider VPS with WireGuard

### Port 51820 Blocked
Some ISPs block common VPN ports:
- Change to port 443: `ListenPort = 443`
- Try port 53: `ListenPort = 53`
- Update firewall and router accordingly
...
Should show your server's IP, not your ISP's.
...
Should show Cloudflare DNS (1.1.1.1), not your ISP's DNS.

## Common UK ISP Issues

### BT: Port Blocking
BT blocks some ports by default:
- Avoid ports 25, 135, 445
- Use 51820 or 443

### Virgin: Upload Throttling  
Virgin throttles uploads after sustained usage:
- Monitor with `vnstat`
- Consider traffic shaping

### Sky: Firewall Interference
Sky Shield can interfere:
- Disable Sky Shield
- Use custom DNS in router
...
### Slow Speeds
1. Check MTU settings: Try `MTU = 1280` in client config
2. Verify CPU usage: `top`
3. Test direct connection speed

### Can't Access Local Network
Add to client `AllowedIPs`:
```ini
...
```bash
# Disable and stop the interface
sudo systemctl disable --now wg-quick@wg0

# Remove package (Ubuntu)
sudo apt -y purge wireguard wireguard-tools

# Remove configuration (make a backup first!)
sudo rm -f /etc/wireguard/wg0.conf /etc/wireguard/server_private.key /etc/wireguard/server_public.key
```

What this does: Stops the VPN, removes packages, and deletes keys/config. Ensure you have backups of any peer configs.

## FAQ
- Q: How do I change the port from 51820?
  - A: Update `ListenPort` in `/etc/wireguard/wg0.conf`, adjust firewall and router port-forward, then `sudo systemctl restart wg-quick@wg0`.
- Q: Can I use a domain instead of an IP for Endpoint?
  - A: Yes, set `Endpoint = vpn.example.com:51820` and keep DNS updated (use DDNS if IP changes).
- Q: Multiple clients?
  - A: Allocate unique keys and /32 IPs per peer (e.g., 10.8.0.2/32, 10.8.0.3/32) and add a [Peer] block per client on the server.

## References
- WireGuard docs: https://www.wireguard.com/
- wg-quick man page: https://manpages.debian.org/wg-quick
- Our UK Port Forwarding guide: ../networking/uk-port-forwarding
